...
class CrawlProxy(object):
    ...
    def crawl_ip3366(self, page_num = 10):
        """
        获取代理 ip3366
        :param page_num:
        :return:
    ...
    def crawl_nimadaili(self, page_num = 10):
        start_url = 'http://www.nimadaili.com/putong/{}/'
        urls = [start_url.format(page) for page in range(1, page_num + 1)]
        for url in urls:
            print('crawl:', url)
            html = self.get_page(url, 'utf-8')
            if html:
                d = PyQuery(html)
                trs = d('tbody tr').items()
                for tr in trs:
                    text = tr.find('td:nth-child(1)').text()
                    ip,port = text.split(':')
                    scheme='http'
                    verify_result = self.verify.verify_proxy(scheme, ip, port)
                    print('校验完成:',verify_result)
                    if verify_result["status"] == '1':
                        proxy = {
                            "scheme": scheme,
                            "ip": ip,
